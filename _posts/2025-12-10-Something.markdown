---
layout: post
title:  "Bosidian Notes Test"
date:   2025-12-10 12:53:04 +0200
categories: NGS proc
---

## Step 1: Read Quality Check - FastQC and MultQC

1. **Specify a working environment, install QC software and navigate to working directory**
   ```
   # Create and Activate the environment:
   conda create --name ngs_QC python=3.12
   conda activate ngs_QC
   
   # Install the software (fastqc and multqc):
   conda install -c bioconda fastqc
   conda install multiqc
   ```

2. **Perform FastQC analysis on all read files**
   ```
   # create a sub-folder named fastqc_results
...
3. **Run MultiQC to look at and compare the FastQC reports**
   [MultiQC](https://multiqc.info) software allows you to visualize FastQC reports that have been generated at scale:
   ```
   # In the folder containing your read files make a sub-folder called multiqc_report
   mkdir multiqc_report
   
   # Create the multqc report
   multiqc ./fastqc_results -o ./multiqc_report 
   ```

4. **Generate a summary table**
  -  Go to the directory where the MultiQC report is saved and click on this to open up the report to look at the data.
  - Use the most relevant summary statistics that speak to the read information and quality for each file
   

## Step 2: Trimming Sequences

## 1. Installation
1. Install [Conda]([Homepage | Anaconda](https://www.anaconda.com/))
